...
### Evidence pack (governance/evidence docs)
- `/docs` — product documentation scaffolding:
  - **System Boundary**: system boundary + modelling assumptions (what is included/excluded and why).
  - **RACI**: stakeholder mapping + RACI to support governance ownership (budgets, policies, incident exceptions).
  - **KPIs**: KPI definitions and reporting cadence (gCO₂e/1k requests, cache hit rate, routing %, p95 latency, feature-level demand signals).
  - **Risk Register**: risk register (rebound, privacy, lock-in, heatwave/outage, monitoring overhead) with mitigation intent.
- `/workbook` — calculation artefacts:
  - `scenario-baseline-improved.csv` (baseline vs improved scenarios; intended for sensitivity expansion).
...
- **Boundary exclusions** (e.g., embodied hardware emissions, full data centre PUE effects, end-user devices) are documented in System Boundary and should be revisited if the solution scope expands.
...
3. Inspect evidence artefacts:
   - `/docs` governance/evidence docs and `/data` scenario CSV align with the product structure and system boundary.
...
- Azure MVP (later, minimal cost): collect **real routing/cache/latency** metrics and update Scenario and Data Evidence with measured values + sensitivity analysis.
